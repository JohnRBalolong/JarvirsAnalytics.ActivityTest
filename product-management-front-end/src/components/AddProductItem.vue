...
<div class="mb-2">
  ...
  <input type="file" name="image" id="image" class="px-2 py-1 h-8 block w-full border shadow-sm text-sm focus:ring-blue-500 focus:border-blue-500 border-black rounded-md" accept="image/png, image/jpeg" @change="handleImageInput">
...
>
  ...
  if (error) {
    ...
    const success = await useProductStore().addProduct(formData);
